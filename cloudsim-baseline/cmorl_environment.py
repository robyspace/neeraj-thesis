...
class CMORLEnvironment(gym.Env):
    ...
    def reset(self, seed: Optional[int] = None) -> Tuple[np.ndarray, Dict]:
        ...
        Returns:
            state: 132-dimensional state vector
    ...
    def _calculate_reward(self, success: bool, selected_dc: str) -> Dict[str, float]:
        ...
        # Green datacenter bonus: Reward for choosing green DCs
        ...
        r_energy = renewable_bonus - 0.5 + green_dc_bonus  # Range: [-0.5, 0.6]

        # Carbon reward: Prefer low carbon intensity
        max_carbon = 500.0
        normalized_carbon = min(dc_state['carbon_intensity'] / max_carbon, 1.0)
        r_carbon = -normalized_carbon + green_dc_bonus  # Range: [-1.0, 0.1]
    ...
    def _get_state(self) -> np.ndarray:
        ...
        # 2.5. Datacenter type indicators (5 dimensions: 1 per DC, binary)
        for dc_id in self.datacenter_ids:
            dc_state = self.datacenter_states[dc_id]
    ...
    def _update_datacenter_states(self):
        ...
        for dc_id in self.datacenter_ids:
            ...
            # Update datacenter type (Green/Brown)
